...
export interface DisplayMessage {
    ...
    workflow: any | null; // Replace 'any' with 'WorkflowData'
...
interface UseMessageManagerProps {
    ...
    chatMode: "default" | "workflow"; // Use ChatMode type
    currentUserId?: string | null; // Optional: if you need to associate messages with a user
...
export function useMessageManager({
    ...
    apiData,
    chatMode,
    // currentUserId, // Uncomment if used
    ...
    const addOptimisticUserMessage = useCallback(
        async (content: string, file?: File | null, tempIdPrefix = "temp-user-") => {
            ...
            if (file) {
                ...
                fileData = {
                    name: file.name,
                    type: file.type,
                    // content: fileContent, // Content can be added here or by the submit hook
                    content: null, // Let submit hook handle content for API
                };
            }
    ...
    const updateOptimisticUserMessage = useCallback(
        async (messageId: string, newContent: string, newFile?: File | null) => {
            ...
            if (newFile) {
                ...
                newFileData = { name: newFile.name, type: newFile.type, content: null /*fileContent*/ };
            }
            ...
            setDisplayMessages(prev => {
                ...
                updatedMessages[messageIndex] = {
                    ...updatedMessages[messageIndex],
                    content: newContent,
                    file: newFile === undefined ? updatedMessages[messageIndex].file : newFileData, // Keep old if undefined, null if explicitly cleared
                };
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        const syncMessagesFromAPI = useCallback((currentApiMessages: VercelMessage[], currentApiData: any) => {
        ...
        setDisplayMessages(prevDisplayMessages => {
            ...
            for (const apiMsg of currentApiMessages) {
                const existingMsgIndex = newMessages.findIndex(dm => dm.id === apiMsg.id);
                ...
                const displayMsgPayload: DisplayMessage = {
                    ...
                    workflow: (apiMsg.role === "assistant" && chatMode === "workflow") ? currentApiData : null,
                    // file: apiMsg.file, // If your VercelMessage includes file info from server
                };
            
                if (existingMsgIndex !== -1) {
                    // Update existing message if content or workflow data changed
                    if (newMessages[existingMsgIndex].content !== content ||
                        (chatMode === "workflow" && newMessages[existingMsgIndex].workflow !== currentApiData)
                    ) {
                    ...
                    const lastOptimisticUserMsg = newMessages.length > 0 && newMessages[newMessages.length -1].role === 'user' && newMessages[newMessages.length -1].id.startsWith('temp-user-');
            
                    if (apiMsg.role === 'user' && lastOptimisticUserMsg && newMessages[newMessages.length -1].content === apiMsg.content) {
            ...
            if (newMessages.length >= 2) {
                ...
                if (last.role === "assistant" && secondLast.role === "assistant" && last.id !== secondLast.id) {
    ...
    }, [chatMode, onDisplayMessagesChange]);
